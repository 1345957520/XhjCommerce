...
namespace Chj.Core
{
    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        protected virtual bool TryWriteGlobalAsax()
        {
            try
            {
                //When a new plugin is dropped in the Plugins folder and is installed into jucango, 
        ...
        public virtual void RestartAppDomain(bool makeRedirect = false, string redirectUrl = "")
        {
            ...
            else
            {
                ...
                if (!success)
                {
                    throw new NopException("jucango needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
        ...
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContext.Items["Chj.IsPOSTBeingDone"] == null)
                    return false;
                return Convert.ToBoolean(_httpContext.Items["Chj.IsPOSTBeingDone"]);
            }
            set
            {
                _httpContext.Items["Chj.IsPOSTBeingDone"] = value;
            }
        }
